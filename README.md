...
The blueprint ships two e2e suites, both aware of the primeng widgets (`p-select`, `p-multiselect`,
`p-datepicker`, `p-checkbox`, `p-fileupload`, `p-confirmdialog`) and of composite keys.

Pick one — or both — through the standard `testFrameworks` application option:
...
| Value        | Generated in                      | Notes                                                           |
| ------------ | --------------------------------- | --------------------------------------------------------------- |
| `cypress`    | `src/test/javascript/cypress/`    | jhipster's own suite, with the primeng selectors patched in     |
| `playwright` | `src/test/javascript/playwright/` | provided by this blueprint, jhipster core has no playwright yet |

`playwright` is not a value jhipster core knows about: the blueprint's `client` sub-generator picks
it up and composes `generators/playwright`. When both are enabled cypress keeps the generic `e2e` /
`e2e:headless` scripts and playwright is reachable through `e2e:playwright*`.

The playwright suite mirrors the cypress one file for file (account, administration and per-entity
CRUD specs). Instead of custom commands it exposes typed fixtures from `support/fixtures.ts`:

- `login` / `api` — authenticate and call the backend with the current credentials (jwt, session or oauth2)
- `network` — `watch()` and `stub()`, the counterparts of `cy.intercept` and `cy.wait('@alias')`
- `navbar`, `entity`, `account`, `management` — page objects, `entity` holding the primeng interactions

The suite runs sequentially (`workers: 1`) because it mutates shared backend data, exactly like the
cypress one. Browsers are installed with `npx playwright install --with-deps chromium`. Only a chromium
project is generated by default; set `playwrightBrowsers` in `.yo-rc.json` (or pass
`--playwright-browsers chromium,firefox,webkit` when running the sub-generator on its own) to generate
more.
